...
- $(AB)^{T}=B^{T}A^{T}$ 
- $(A^{-1})^{T}= (A^{T})^{-1}$
Soit $S$ une matrice symétrique : $S^{T}=S$.
Soit $A$ une matrice antisymétrique : $A^{T}=-A$.
...
- Si on trouve le même nombre de [[Espaces de dimension finie#Pour un système linéaire|rang]] et de pivot, alors $A^{-1}$ existe.
