...
On considère une OPPH polarisé selon  $Oy$ se déplasant selon $x$ croissant :
...
On prend $\overrightarrow{u_{trav}}= - \overrightarrow{e_{x}}$ en prenant la variation du champ de l'intérieur ve rs l'extérieur.
### Onde réfléchi
...
Onde stationnaire $\overrightarrow{E} = f(x)g(t)\overrightarrow{e_{y}}$ 
