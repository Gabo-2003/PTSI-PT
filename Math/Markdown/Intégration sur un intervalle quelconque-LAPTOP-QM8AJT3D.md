...
$\displaystyle \int_{I} f$ converge $\displaystyle \Leftrightarrow \begin{cases} \int_{I} f \underset{t \to  a } \longrightarrow \text{  limite finie  } \\ \text{ou} \\  \text{  Soit  } F \text{   une primitive de  }f, F(t) \underset{t \to  a } \longrightarrow \text{  limite finie  }\end{cases}$
### Propriétés 
Soit $I \subset \mathbb{R}$ une intervalle avec une seule singularité  $a \in \overline{\mathbb{R}}$.
#### Relation de Chasles
...
- Si $\int_{a}^{b} f$ converge, alors $\displaystyle \int_{a}^{b} f = \int_{a}^{c} f + \int_{c}^{b} f$
#### Linéarité 
...
- $\displaystyle \int_{I}f$ converge $\Leftrightarrow \int_{I}Re(f) \text{  et  } \int_{I} Im(f)$ convergent
...
$\displaystyle \int_{I} f$ converge 
si $\begin{cases} f<g \text{  et  } \int_{I} g \text{  converge  }  \\ \text{ou} \\ f \underset{a } \sim g \text{  et  } \int_{I} g \text{  converge  }  \\ \text{ou} \\ f \underset{+ \infty } = o(g) \text{  ou  } f \underset{+ \infty } = O(g)\text{  et  } \int_{I} g \text{  converge  } \end{cases}$
...
Soit $f$ continue sur $]a,b[$ , on pose $t=\varphi(u)$  $\varphi : ]\alpha , \beta [ \longrightarrow ]a,b[$ strictement monotone, de classe $C^{1}$ et bijective. 
Alors $\displaystyle \int_{a}^{b} f(t) \: d{t}$ de même nature que $\displaystyle \int_{\alpha}^{\beta } f(\varphi (u)) \: \varphi'(u) d{u}$
Et si convergentes, $\displaystyle \int_{a}^{b} f(u) \: d{t} = \int_{\alpha}^{\beta } f(\varphi (u)) \: \varphi'(u) d{u}$
...
### Singularité en $0$ 
- $\displaystyle\int_{0}^{1} \frac{1}{t^{\alpha }} = \frac{1}{1-\alpha }$ donc converge $\Leftrightarrow \alpha >0$ 
...
- $\displaystyle t \mapsto \frac{1}{|t-a|^{\alpha }}$ intégrable en $a \Leftrightarrow \alpha < 0$ 
