...
## Éléments propres 
### Valeurs propres 
Soit $f \in  \mathcal{L}(E)$ et $\lambda \in \mathbb{K}$ avec $\dim(E) = n$. 
$\lambda$ est une valeur propre de $f$ : 
$\Leftrightarrow \exists x \in E \; ; \quad f(x) = \lambda x$   avec $x \neq 0_{E}$ 
$\Leftrightarrow x \in \ker(f - \lambda id_{E}) \Leftrightarrow \ker(f- \lambda id_{E}) \neq \{ 0_{E} \} \Leftrightarrow f- \lambda id_{E}$ n'est pas injective 
$\Leftrightarrow \dim(\ker(f- \lambda id_{E})) \neq 0$ qui par théorème du rang $\Leftrightarrow \dim(im(f- \lambda id_{E})) = rg(f - \lambda id_{E}) \neq rg_{max} = n$ 
$\Leftrightarrow \det(f - \lambda id_{E}) = 0$
- On peut échanger $\ker(f - \lambda id_{E})$ par $\ker(\lambda id_{E} - f)$ 
### Vecteurs propres 
