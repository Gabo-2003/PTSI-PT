...
- $Mat_{B}(u_{1}, \ldots, u_{p})$ matrices dont la j-èm colonne est les coordonnées de $u_{j}$ dans $\mathcal{B}$.   
Soit $u \in \mathcal{L}(E,F)$, $E$ de base $\mathcal{B}=(e_{1}, \ldots, e_{p})$, $F$ de base $\mathcal{C}=(f_{1}, \ldots, f_{n})$.
- $Mat_{\mathcal{B},\mathcal{C}}(u)$ matrice dont la j-èm colonne est les coordonnées de $u(e_{j})$ dans $\mathcal{C}$. 
- Alors ce la matrice de la famille $u(\mathcal{B}) = (u(e_{1}), \ldots, u(e_{p}))$ dans la base $\mathcal{C}$.
![[image matrice dune application lineaire.png]]
...
- $X_{B}=P_{BB'} X_{B'} \Rightarrow X=PX'$ 
- $X_{B'}=P_{B'B} X_{B} \Rightarrow X'=P^{-1} X$ 
