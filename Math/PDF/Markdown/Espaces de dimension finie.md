...
## Rang
### Famille de vecteurs 
C'est la dimension du sev engendrée par cette famille.
$rg(u_{1}, u_{2}, \ldots, u_{p})= \dim(vect(u_{1}, \ldots, u_{p}))$ 
- Si la famille est [[Espaces vectoriels#Famille libre|libre]] : $rg(u_{1}, u_{2}, \ldots, u_{p})=p$. 
### Application linéaire 
C'est la dimension de son image. 
$rg(f) = \dim(Im(f))$
#### Théorème du rang
$\dim(E)=\dim(\ker(u))+\dim(Im(u))$ 
### Système linéaire 
C'est est le nombre d'équations qui compte tout système échelonné équivalent. 
...
Soit un $\mathbb{K}.e.v$ de $E$ de base $\mathcal{B}=(\overrightarrow{e_{1}}, \ldots, \overrightarrow{e_{p}})$.
Soit $(y_{i})_{1\leq i \leq p}$ une famille de $p$ vecteurs d'un $\mathbb{K},e,v \; F$. 
$\exists! f :E \longrightarrow E$ tel que $\forall i,  f(e_{i})=y_{i}$. 
