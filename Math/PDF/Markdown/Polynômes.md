...
- $\displaystyle AB= \sum_{k=0}^{n} a_{k} b_{n-k} X^{k}$  
- $\deg(AB)=\deg(A)+\deg(B)$
...
## Racines
On a $\forall a \in \mathbb{K}$ et $P \in \mathbb{K}[X]$ :   $P=Q(X-a)+\tilde{P}(a)$.
$a$ est une racine de $P$ :
$\Leftrightarrow \tilde{P}(a) = 0 \Leftrightarrow P = Q(X-a)$ 
### Ordre de multiplicité
Le plus grand $m$ tel que $P$ divisible par $(x-a)^{m}$.
### Polynôme scindé
Un polynôme est scindé si il peut se mètre sur la forme de produits de polynômes de degré $1$ :
$P = \lambda (X-a_{1})(X-a_{2})\ldots(X-a_{n})$ 
- $a_{n}$ les racines du polynôme.
- $\lambda$ le coefficient dominant. 
...
$\displaystyle P = \sum_{k \in \mathbb{N}} \tilde{P}^{(k)}(\alpha) \frac{(X- \alpha)^{k}}{k!}$  
### Formule de Mac-Laurin  
$\displaystyle P= \sum \tilde{P}^{(k)}(0) \frac{X^{k}}{k!}$ 
