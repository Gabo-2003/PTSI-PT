...
Soit le rapport $\displaystyle \frac{f(x)}{g(x)}$ lorsque $x \longrightarrow a$.
...
- $f \underset{+ \infty} = o(g) \Rightarrow f \underset{+ \infty} = O(g)$
### Propriétés 
...
$\quad \quad \begin{cases} f \times h \underset{a}\sim g \times \phi \quad \text{et} \quad \displaystyle \frac{f}{h} \underset{a} \sim \frac{g}{\phi} \\ (f(x))^{\alpha} \underset{a}\sim (g(x))^{\alpha} \quad \alpha \in \mathbb{R} \text{  fixé} \\ \text{Il es faux :  } \cancel{f+h \sim g+ \phi, \quad \psi(f(x)) \underset{a}\sim \psi(g(x))}    \end{cases}$
